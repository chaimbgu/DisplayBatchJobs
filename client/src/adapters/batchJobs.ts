...
const getBatchJobsDetails:(id: string)=> Promise<BatchJobDetails> = async (id) => {

     return (await axios.get(`http://localhost:5000/getJobDetails/${id}`)).data
    }

    const addJob  = async () => {
         axios.post(`http://localhost:5000/addBatchJob`, {myData : "3"})
       }



export const useGetBatchJobsList = ()=>{
    return     useQuery({queryKey : ["jobsList"], queryFn: async ()=>{
...
export const useGetBatchJobsDetails = (jobId : string)=>{
    return      useQuery({queryKey : [`jobDetails${jobId}`], queryFn: async ()=>{
